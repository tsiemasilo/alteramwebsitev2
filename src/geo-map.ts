...
class SouthAfricaMap {
  ...
  private resize(): void {
    const container = d3.select('#geo-map-container').node() as HTMLElement;
    if (!container) return;

    this.width = container.clientWidth;
    this.height = Math.min(this.width * 0.9, 1800);
  ...
  private render(): void {
    ...
    locations.forEach(location => {
      ...
      if (location.number) textElements.push(location.number.toString());
      textElements.push(location.name);
      if (location.subtitle) textElements.push(location.subtitle);
  ...
  private getLabelOffset(position: string): { x: number, y: number } {
    const offset = 35;
  ...
  private getTextAnchor(position: string): string {
    if (position.includes('left')) return 'end';
    if (position.includes('right')) return 'start';
    return 'middle';
  }

  private onMarkerHover(marker: d3.Selection<SVGGElement, unknown, HTMLElement, any>, isHover: boolean): void {
  ...
  private onMarkerClick(locationId: string, marker: d3.Selection<SVGGElement, unknown, HTMLElement, any>): void {
